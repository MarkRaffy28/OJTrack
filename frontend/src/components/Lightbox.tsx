  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      try {
        ...
        console.error("File preparation failed, falling back to URL sharing:", err);
      }
  ...
  return (
    ...
    >
      <div className="lb-header" onClick={e => e.stopPropagation()}>
        <button className="lb-action-btn" onClick={handleDownload} title="Download">
      ...
      <div className="lb-hint">
        {scale > 1 ? "Drag to pan · Pinch to zoom out" : "Pinch or double tap to zoom"}
      </div>
    </div>
  );
};
