...
@section ("header_title", "Log Attendance")

@section ("header_description", "Manually record student OJT attendance.")
...
@section ("content")
  <div class="mx-auto max-w-2xl">
    <div class="rounded-xl border border-border bg-surface p-6 shadow-xs">
      <form method="POST" action="{{ route("web.{$role}.attendance.store") }}" class="space-y-6">
        @csrf
        ...
        <div>
          ...
          <select name="ojt_id" required class="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-text focus:outline-none focus:ring-2 focus:ring-primary-500">
            ...
            @foreach ($studentOjts as $ojt)
              <option value="{{ $ojt->id }}" @selected(old('ojt_id') == $ojt->id)>
        ...
        <div class="rounded-lg border border-border bg-background p-4 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="time" name="morning_in" value="{{ old('morning_in') }}" class="w-full rounded-lg border border-border bg-surface px-3 py-1.5 text-sm text-text" />
              <label class="mt-2 inline-flex items-center gap-2 text-xs text-text">
                <input type="checkbox" name="morning_in_verified" value="1" @checked(old('morning_in_verified')) class="rounded border-border text-primary-600" />
                <span>Approved</span>
              </label>
            </div>
            ...
            <div>
              ...
              <input type="time" name="morning_out" value="{{ old('morning_out') }}" class="w-full rounded-lg border border-border bg-surface px-3 py-1.5 text-sm text-text" />
              <label class="mt-2 inline-flex items-center gap-2 text-xs text-text">
                <input type="checkbox" name="morning_out_verified" value="1" @checked(old('morning_out_verified')) class="rounded border-border text-primary-600" />
                <span>Approved</span>
              </label>
            </div>
          </div>
        ...
        <div class="rounded-lg border border-border bg-background p-4 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="time" name="afternoon_in" value="{{ old('afternoon_in') }}" class="w-full rounded-lg border border-border bg-surface px-3 py-1.5 text-sm text-text" />
              <label class="mt-2 inline-flex items-center gap-2 text-xs text-text">
                <input type="checkbox" name="afternoon_in_verified" value="1" @checked(old('afternoon_in_verified')) class="rounded border-border text-primary-600" />
                <span>Approved</span>
              </label>
            </div>
            ...
            <div>
              ...
              <input type="time" name="afternoon_out" value="{{ old('afternoon_out') }}" class="w-full rounded-lg border border-border bg-surface px-3 py-1.5 text-sm text-text" />
              <label class="mt-2 inline-flex items-center gap-2 text-xs text-text">
                <input type="checkbox" name="afternoon_out_verified" value="1" @checked(old('afternoon_out_verified')) class="rounded border-border text-primary-600" />
                <span>Approved</span>
              </label>
            </div>
          </div>
