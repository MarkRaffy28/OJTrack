...
export const getAttendanceByIdController = async (req, res) => {
  const data = validate(res, getAttendanceByIdSchema, req.params);
  if (!data) return;

  const { id } = data;

  const row = await fetchOrFail(
    res,
    getAttendanceByIdRow,
    [id],
...
export const getTodayAttendanceController = async (req, res) => {
  ...
  const dbRecord = await fetchOrFail(
    res,
    getTodayAttendance,
...
export const scanAttendanceController = async (req, res) => {
  ...
  if (
    !(await fetchOrFail(
      res,
      isStudentInOffice,
      [studentId, qrPayLoad.o],
      "Student not assigned to this office",
    ))
  )
    return;
  ...
  if (
    (column === "morning_out" && (!record || !record["morning_in"])) ||
  ...
  await startOjtIfPending(ojtId);
  await recalculateRenderedHours(ojtId);

  const action = column.endsWith("_in") ? "TIME_IN" : "TIME_OUT";
  const sessionStr = column
    .replaceAll("_", " ")
